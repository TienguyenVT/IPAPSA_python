...
# 2. Add @njit
code = code.replace("def levinson_durbin(", "@njit(cache=True)\ndef levinson_durbin(")
code = code.replace("def ar_filter(", "@njit(cache=True)\ndef ar_filter(")
code = code.replace("def sign(", "@njit(cache=True)\ndef sign(")
code = code.replace("def soft_sign(", "@njit(cache=True)\ndef soft_sign(")
...
# Replace process_sample and process_chunk
code = re.sub(r'    def process_sample\(self.*?(?=    def process_chunk)', '', code, flags=re.DOTALL)
code = re.sub(r'    def process_chunk\(self.*?(?=    def reset)', new_process_chunk, code, flags=re.DOTALL)

code = code.replace("class AdaptiveFilter:\n", numba_func)

with open('adaptive_algorithms.py', 'w', encoding='utf-8') as f:
    f.write(code)

print("Refactored adaptive_algorithms.py")
